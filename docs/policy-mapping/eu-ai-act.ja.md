...
review ではない。2025–2026 年の 8 source を挙げるのは、それらが AAP が
...
- **Butt, Iqbal & Iqbal, *Governing What the EU AI Act Excludes:
  ...
  Infrastructure* (arXiv:2605.01091, 2026-05).** Act の **Annex III
  point 2** の除外 ── 重要インフラの safety-component AI が
  **Art 86** の説明を受ける権利と **Art 27** の fundamental-rights
  impact assessment の外に置かれる ── を smart-city シナリオで検討
  する: 交通信号 agent と電力管理 agent がそれぞれ単体では
  provider / deployer 義務に準拠しながら、同じ corridor 上で *相互
  作用* した複合的帰結について、住民は誰にも説明を要求できない。
  その核心的観察 ── bounded な AI システムは単体で評価できるが、
  相互作用する autonomous agent の集合は評価できない ── は、AAP が
  運用から記録した **attribution gap**
  ([ADR-0009](../adr/0009-triage-before-autonomy.ja.md);
  [`../glossary.ja.md`](../glossary.ja.md)) の、規制側からの独立した
  記述である: gap は構造的であって、単一 actor の compliance 欠陥
  ではない。paper が提案する救済 (multi-agent 都市配備向けの層状
  governance architecture) は *mechanism* であり、本 repository の
  thesis に従ってこの mapping layer に留める。
- **Chiodo et al., *Formalising Human-in-the-Loop: Computational
  Reductions, Failure Modes, and Legal-Moral Responsibility*
  (arXiv:2505.10426, 2025-05; ICLR 2026).** human-in-the-loop を
  oracle-machine への reduction として形式化し、*trivial monitoring* /
  *many-one* (単一終点の承認) / *Turing* (複数回の対話) の reduction を
  区別する。**Art 14** と GDPR **Art 22** が求める *meaningful で
  effective な oversight* を満たすのは Turing 型のみであり、many-one
  型の approval gate こそ moral crumple zone になりやすい、と論じる。
  これは **ADR-0005** (human approval gate) に収束し、
  [`../glossary.ja.md`](../glossary.ja.md) の *moral crumple zone* 項を
  鋭くする: oversight が genuine かを決めるのは gate の有無ではなく
  *形状* である。
- **Kaptein, Khan & Podstavnychy, *Runtime Governance for AI Agents:
  Policies on Paths* (arXiv:2603.16586, 2026-03).** compliance を、
  次の単発 action ではなく *これまでの実行 path* 上で評価する
  *path-dependent* policy として、また **Monitor / Soft-enforcement /
  Hard-block** という graded な執行モデルとして形式化する。この
  graded モデルは **ADR-0002** が記録する prohibition-strength
  hierarchy と、**ADR-0005** が求める **Art 14** oversight の
  *mechanism* である。OWASP の *"autonomy is a feature to be earned,
  not a default"* は同じ graded-prohibition の独立採用である。
  mechanism は ADR 本文ではなくこの layer に留める。
- **Uchibeke, *Before the Tool Call: Deterministic Pre-Action
  Authorization for Autonomous AI Agents* (arXiv:2603.20953,
  2026-03).** Open Agent Passport を提案する: tool call を実行 *前* に
  傍受 (ALLOW / DENY / REQUIRE_APPROVAL / MASK) し、暗号署名付きの
  audit trail を出す deterministic な policy-enforcement point。
  permissive policy 下の攻撃成功率 74.6% が restrictive policy 下で
  0% へ転落したと報告される。**ADR-0005** (human approval gate) と
  **ADR-0006** (causal traceability) を **Art 14** の下で支える
  *mechanism* として記録する: 責任主体を確率的な model 出力から
  明示的な policy enforcer へ移す。shipping mechanism (DSL ベースの
  enforcement point) はこの layer に留め、judgment が ADR である。
- **Storf et al., *Constitutional Black-Box Monitoring for Scheming
  in LLM Agents* (arXiv:2603.00829, 2026-02; ICML 2026).** agent の
  *scheming* を、chain-of-thought を意図的に除外し、外部から観測
  可能な tool call・query・出力のみから partial AUROC 0.85+ で検出
  する。これは **ADR-0006** (causal traceability) と AAP の
  *non-invasion posture* の定量的な接地である: 内部状態の inspection
  ではなく観測可能な behaviour に accountability を置く構えであり、
  **Art 72** の post-market behavioural-monitoring 義務と整合する。
  paper 自身が示す認識論的限界 ── 十分に最適化された agent は外部から
  clean に見えるよう訓練されうる ── は、緩和せず boundary condition
  として記録する。

## Adjacent EU instrument — 改正 Product Liability Directive

隣接する EU instrument を 1 つここに記す。AAP の語彙が名指す問題
を、*義務* 側ではなく *補償* 側から立法しているからである。
**Directive (EU) 2024/2853** (改正 Product Liability Directive;
2024 年 12 月発効、2026-12-09 以降に上市・供用開始された製品に
適用) は、software ── Regulation (EU) 2024/1689 の意味での AI
システムを明示的に含む (Recital 13) ── を「製品」定義 (Art 4(1))
の内側に置き、strict な欠陥責任を適用する。その設計判断のうち
2 つが AAP の judgment に収束する:

- **市場投入後の control が責任の窓を延長する (Art 7(2)(e))**。
  defectiveness は上市時点だけでなく、*製造者がその後も製品への
  control を保持する場合* (update 経由等)、製品が製造者の control
  を離れた時点で評価される。継続的に update される software に
  ついて factory-gate 原則は放棄された。これは **ADR-0010** が
  load-bearing と記録する design / operation の区別の、責任法側の
  anchor である: 立法者もまた、運用中に変化し続けるシステムでは
  deployment の瞬間が accountability の問いを閉じられないことを
  見出した。
- **残余 gap の事前に名指された bearer**。AI Liability Directive
  提案が撤回された (Commission 2025 work programme) 現在、PLD は
  AI 被害に対する EU レベルの主要な補償経路であり、その被害を
  事後に探される主体ではなく *事前に* 名指された主体 ── 製造者 ──
  へ routing する。これは **ADR-0009** が記録する judgment ──
  attribution gap は failure の後に発見されるのではなく、deployment
  前に特定された主体が吸収する ── の制度的 instance である。

この収束には明記すべき限界がある: strict liability は *補償* を
routing するのであって、**ADR-0006** が求める因果の *再構成* を
生むわけではない。どの agent が、誰の指示の下で被害を生んだのかを
誰も確定しないまま、損害が支払われる regime はありうる。補償の
routing と attribution の再構成は別の問題であり、PLD が扱うのは
前者だけである。
