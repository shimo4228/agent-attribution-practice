...
- **失敗モード。** Autonomous Agentic Loop Quadrant 部品が operation
  ...
  Phase-crossing decision（フェーズ越境判定）が記録されていない。
  ADR-0010 が表に出す (ban ではなく) configuration — 意識的に選ぶ
  べきで、滑り込ませるものではない。
- **なぜ起きる。** 「自律エージェントが業務 workflow を handle」が
  ...
  silently に loop に absorb され、design-phase feedback として
  surface しない。
- **何が壊れる。** [`attribution gap`](../glossary.ja.md#attribution-gap寄与の事後分離不能ギャップ)
  が production runtime 内で発火する。動的に absorb された各新パターンが
  attribution のブレンドをさらに進め、部品レベル redirect が閉ざされる
...
- **回復。** Routing-back-to-design path を明示し log する。Operation
  ...
  escalate、既存 workflow に組み込む (LLM Workflow Quadrant) か、
...
- **なぜ起きる。** 業界の語彙に LLM Workflow Quadrant の肯定形の
  名前がない。「決定論的でないもの」が消去法で「agent」に mapping
...
- **何が壊れる。** 機能すべき部品レベル redirect が機能しなくなる。
  ...
  切り分けられない。チームは誤った象限のアカウンタビリティ性質を
  仮定する — redirect が可能だと assume するが、すでに閉ざされている。
- **症状。** Sandbox 強度がエスカレートする。Human-in-the-loop の負荷
  が増える (operator が system が redirect できない判断を抱え込む)。
...
- **何が壊れる。** 解の品質が古典探索より悪化。Loop が feasibility を
  hallucinate する。Objective function がなく、LLM が drift する
...
- **何が壊れる。** Cost、レイテンシ、信頼性すべてが利益なしで劣化する。
  決定性が失われる (loop が run 間で異なる出力を出す)。Audit が
  置き換え対象のスクリプトより難しくなる。
...
- **失敗モード。** Autonomous Agentic Loop Quadrant 業務が pre-named
  gap-bearer（事前命名された責任引受者）なしで動いている。ADR-0009
  が防ぐべき configuration。
...
- **失敗モード。** ADR-0005 が ADR-0001 critique の意味での signpost
  に縮小。
- **なぜ起きる。** Approval gate が confirmation dialog と取り違え
  られる。後者は habituate するが、前者は named human による inspect
  と decide が要る。
- **何が壊れる。** Operator がクリック through する。人間の decision
  で終端すべき chain が「user が Enter を押した」で終端する。
  Post-incident review が実 sign-off を発見できない。
...
- **なぜ起きる。** Logging cost の懸念; 「全部読む人いる?」 Audit log
  が monitoring data として扱われ、ADR-0006 が named する構造的
  artifact ではなくなる。
- **何が壊れる。** Incident が起きた時、関連 run が sample にない。
...
- **回復。** すべての event に append-only logging。Storage cost は
  ...
  deployment が Autonomous Agentic Loop Quadrant にある時に遥かに
  高い。
...
- **何が壊れる。** 同じ agent process が異なる象限の業務に deploy
  されうる。Agent を象限に lock すると triage が隠れる。Agent が
  異なる象限の業務を依頼された時、アーキテクチャ mismatch が
  invisible になる。
- **回復。** Quadrants は *業務* を述べる。Agent が依頼される各業務
  piece に decision tree を実行する。同じ agent process でも異なる
  象限は異なる適用 ADR set を要求する。
...
- **なぜ起きる。** Cognitive shortcut。答えが obvious に見える時に
