...
> Autonomy](../adr/0009-triage-before-autonomy.md) と ADR-0010
> [Phase Separation](../adr/0010-phase-separation.md) を 5 質問の
...
Q1–Q4 は [ADR-0009 の problem-space triage](../adr/0009-triage-before-autonomy.md)。
Q0 は [ADR-0010](../adr/0010-phase-separation.md) の Phase-crossing
decision を、業務が operation phase かつ Q1–Q4 の結果が Autonomous
Agentic Loop Quadrant のときに surface する。

Phase と Quadrant は **独立な dimension** — 各 Quadrant は両 phase に
出現しうる。Q0 は Quadrant フィルタではなく、Phase-crossing decision
が load-bearing になる場面を表に出すための prompt。
...
- **Operation phase** (deploy された workflow が既知経路上を走り、
  ...
  (+ 2 where applicable) だが、これは default であって requirement
  ではない。

Phase の判断自体を deployment alongside で記録する。「これは
operation phase、default composition (Quadrant 1 + 3)」は完全な記録。
「これは operation phase、Quadrant 4 admitted、Phase-crossing
decision: 新パターンを design に戻す」も完全な記録。「決めなかった」
は不完全。
...
互いに相談せずに同一の出力を produce できるか? Yes なら意味判断は
不要。
...
- **Yes →** [`Script Quadrant`](README.ja.md#1-script-quadrant)。
  スクリプト、パイプライン、ワークフローエンジンを使う。本 repository
  の 10 本の ADR は概ねこの業務の射程外。
- **No →** [`Algorithmic Search Quadrant`](README.ja.md#2-algorithmic-search-quadrant)。
  古典探索、動的計画法、MCTS、強化学習、整数計画、制約ソルバを使う。
  本 repository の LLM 中心の ADR の射程外。
...
- **Yes →** [`LLM Workflow Quadrant`](README.ja.md#3-llm-workflow-quadrant)。
  決定論的制御フロー + 名前と documented role + 明示 input/output
  schema を持つ bounded LLM 呼び出しの組み合わせを使う。これが現行
  LLM 応用の default。**適用 ADR:**
  - [ADR-0001](../adr/0001-security-by-absence.md) — codebase に
    存在しない capability は呼び出せない
  - [ADR-0003](../adr/0003-untrusted-content-boundary.md) — 蓄積
    state は権威を与えない
  - [ADR-0004](../adr/0004-single-external-adapter.md) — blast radius
    を構造で限定
  - [ADR-0005](../adr/0005-human-approval-gate.md) — 行動変更は
    名前のある人間の承認を要する
  - [ADR-0006](../adr/0006-causal-traceability.md) — あらゆるイベント
    が事後再構成可能
  - [ADR-0007](../adr/0007-scaffolding-visibility.md) — 振る舞いは
    ファイルに存在
- **No →** Q4 に進む。
...
### Q4. Pre-named gap-bearer を identify できるか? かつ (Q0 = operation の場合) Phase-crossing decision は記録されているか?

ここまで来た業務は本当に **Autonomous Agentic Loop Quadrant** を要求
している — 経路が事前予測不能で、LLM 呼び出しの役割を bounded にする
と業務遂行を妨げる。アーキテクチャは runtime で判断要素を blend する;
事後分離可能性は foreclose され、分離可能寄与のレベルでの redirect は
機能しない。[`attribution gap`](../glossary.ja.md#attribution-gap)
は intrinsic。
...
(*moral crumple zone* 失敗モード)。

**Part B (Q0 = operation phase の場合のみ)。** Phase-crossing
decision は記録されているか — operation 中に新パターンが現れたとき、
...
- **Yes (両 part) →** [`Autonomous Agentic Loop Quadrant`](README.ja.md#4-autonomous-agentic-loop-quadrant)。
  **10 本の ADR すべて** を適用; ADRs 0005, 0006, 0008, 0009, 0010 が
  最も load-bearing (これらが named gap-bearer の commitment と
  lifecycle 配置を operational にする)。Gap-bearer (と該当時は
  Phase-crossing decision) を architectural choice alongside で
  deployment 時に記録。
...
が、それは象限の境界が structural choice であり、Autonomous Agentic
Loop 部分が named gap-bearer 付きで扱われる場合のみ。
...
- **Phase 移行**。Design phase で始まる業務は経路が判明するに従って
  ...
  (regression ではなく legitimate maturation)。
...
- **Phase-crossing decision の変更**。「新パターンを design に戻す」
  ...
  architectural change alongside で再記録。
- **規制の変化**。Q4 の答え (該当ドメインで gap-bearer が指名できるか)
  を変えうる。

迷ったら 5 質問を再実行し、新しい答えを architectural change
alongside で文書化する。
