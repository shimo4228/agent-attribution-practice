...
- **Yes →** [`LLM Workflow Quadrant`](README.ja.md#3-llm-workflow-quadrant)。
  ...
  - [ADR-0001](../adr/0001-security-by-absence.ja.md) — codebase に
    存在しない capability は呼び出せない
  - [ADR-0003](../adr/0003-untrusted-content-boundary.ja.md) — 蓄積
    state は権威を与えない
  - [ADR-0004](../adr/0004-single-external-adapter.ja.md) — blast radius
    を構造で限定
  - [ADR-0005](../adr/0005-human-approval-gate.ja.md) — 行動変更は
    名前のある人間の承認を要する
  - [ADR-0006](../adr/0006-causal-traceability.ja.md) — あらゆるイベント
    が事後再構成可能
  - [ADR-0007](../adr/0007-scaffolding-visibility.ja.md) — 振る舞いは
    ファイルに存在
- **No →** Q4 に進む。
...
と業務遂行を妨げる。アーキテクチャは runtime で判断要素をブレンドする;
事後分離可能性は閉ざされ、分離可能な寄与のレベルでの redirect は
機能しない。[`attribution gap`](../glossary.ja.md#attribution-gap寄与の事後分離不能ギャップ)
は本質的に内在する。
...
- **Yes (両 part) →** [`Autonomous Agentic Loop Quadrant`](README.ja.md#4-autonomous-agentic-loop-quadrant)。
  **10 本の ADR すべて** を適用; ADRs 0005, 0006, 0008, 0009, 0010 が
  ...
  lifecycle 配置を運用可能にする)。Gap-bearer (と該当時は
  Phase-crossing decision) を architectural choice と並べて
  deployment 時に記録。
