...
各象限の operational signal は
[`quadrants/README.ja.md`](../quadrants/README.ja.md#4-つの象限) を、
本 Decision を operational 化した 5 質問 triage は
[`quadrants/decision-tree.ja.md`](../quadrants/decision-tree.ja.md)
を参照。
...
ADR-0009 は適用順序で ADRs 0001–0008 の手前に位置する — 与えられた
業務に対して 8 本のうちのどれが load-bearing になるかを決める triage。
象限ごとの ADR 適用表は
[`quadrants/governance-mapping.ja.md`](../quadrants/governance-mapping.ja.md#象限ごとの詳細)
を参照。Summary: Script / Algorithmic Search Quadrants は射程外;
LLM Workflow Quadrant では ADRs 0001–0007 (+ external side effects
時は 0008); Autonomous Agentic Loop Quadrant では *9 本すべて*、
ADR-0005、ADR-0006、ADR-0008 が最も weight を持つ。
...
した。先行 essay の暫定命名 ("ReAct Quadrant"、"Classical AI Quadrant")
は [`../glossary.ja.md`](../glossary.ja.md) で alias として保存される;
