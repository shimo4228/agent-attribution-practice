Language: English | [日本語](0001-security-by-absence.ja.md)
...
## Date
2026-04-09 (decided) / 2026-04-19 (re-expressed in this repository)
...
This project adopts **Security by Absence** as a first-class design principle:
...
| Arbitrary outbound HTTP | Absent (single pinned adapter only; see ADR-0004) |
...
- **Data exfiltration through the pinned adapter** — if a single external
  adapter exists (ADR-0004), content written through it can still leak.
...
3. If the capability is genuinely unavoidable, the prohibition drops to
   ADR-0002 (Deterministic Prohibition at the Scaffolding Layer). For
   external side effects, the capability must additionally go through
   ADR-0004 (Single External Adapter) and ADR-0005 (Human Approval Gate).
...
- The harness ships as specifications, schemas, and a minimal reference example —
...
**Lineage.** The threat model first appeared in [contemplative-agent `docs/adr/0007-security-boundary-model.md`](https://github.com/shimo4228/contemplative-agent). It was generalized into a standalone principle in [Agent Knowledge Cycle](https://github.com/shimo4228/agent-knowledge-cycle) v1.x, then extracted into this repository in 2026-04-19 when AKC v2.0.0 re-positioned itself as mechanism-only.
