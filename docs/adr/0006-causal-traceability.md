...
(2026-04-13, Article 2 of the trilogy). See
[`../inspiration.md`](../inspiration.md) for the trilogy and the
architectural-follow-up spine context.

**Industry instantiation (informational, 2026 Q2).** [Microsoft Purview AI Data Security Investigations](https://learn.microsoft.com/en-us/microsoft-agent-365/overview), shipped as part of [Microsoft Agent 365](https://www.microsoft.com/en-us/security/blog/2026/05/01/microsoft-agent-365-now-generally-available-expands-capabilities-and-integrations/) (GA 2026-05-01), correlates logs across Microsoft 365, Azure, and third-party AI services to reconstruct the data lifecycle of AI interactions, with automatic AI-sensitive information classification, retention labels, and DLP policy application. As a cross-vendor mechanism instance for this ADR's causal traceability requirement, it is the closest commercial product to the "every event reconstructible after the fact" property. It does not provide the argument for *why* causal traceability has to be a build-time structural requirement rather than a runtime observability practice (this ADR's core point), and it does not record which approval gated which scaffolding version (the audit trail tied to ADR-0005 that the reconstruction needs to terminate at a named human). The two layers are complementary: Purview makes the cross-vendor reconstruction technically possible, this ADR records why the reconstruction has to terminate where it terminates. See [`../inspiration.md`](../inspiration.md#industry-artifacts-that-instantiate-aap-principles).
